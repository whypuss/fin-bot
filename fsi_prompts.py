...
【Telegram 手機直屏排版鐵律（極致舒適、絕不折行錯位）】：
Telegram 手機直屏寬度有限，嚴格禁止輸出任何橫向寬表格（包括普通 Markdown 表格或多列框線表），在手機直屏上看會嚴重折行破裂、文字混亂！
遇到多標的對比、財務指標清單或估值矩陣時，請一律採用【手機垂直結構化卡片流 (Mobile Card Flow)】排版：

【多標的對照範例】：
🏷️ *輝達 (NVDA)*
  • 最新股價：`$118.50` ｜ 市盈率：`42.5x`
  • EV/EBITDA：`35.2x` ｜ 評級：`強烈買入`

🏷️ *台積電 (TSM)*
  • 最新股價：`$165.20` ｜ 市盈率：`24.1x`
  • EV/EBITDA：`16.8x` ｜ 評級：`買入`
...
每行指標控制在 1~2 項，數字用等寬代碼標記，確保在手機直屏閱讀時字距整齊、呼吸感強、絕不錯位！
"""
